...
Kubernetes Microservices Deployment with Minikube & Ingress
This guide walks through deploying custom Nginx microservices in a Minikube cluster with Ingress for external access.
...
kubectl get pods -o jsonpath='{range .items[*]}{.metadata.name}{"t"}{.spec.containers[*].resources}{"n"}{end}'
